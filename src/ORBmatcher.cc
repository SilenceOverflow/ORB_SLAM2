...
int ORBmatcher::SearchByBoW(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches12) {
    ...
    while(f1it != f1end && f2it != f2end) {
        // for the same node in voc tree
        if(f1it->first == f2it->first) {
            // search each local feature in KF1 associated to this node
            for(size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
                ...
                // search each local feature in KF2 associated to this node
                for(size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++) {
                    const size_t idx2 = f2it->second[i2];

                    MapPoint* pMP2 = vpMapPoints2[idx2];

                    // If we have already matched or there is no valid MP
    ...
    } // end of while(f1it != f1end && f2it != f2end)
...
int ORBmatcher::SearchForTriangulation(KeyFrame *pKF1, KeyFrame *pKF2, cv::Mat F12,
    ...
    while(f1it!=f1end && f2it!=f2end) {
        if(f1it->first == f2it->first) {  // same node in voc tree
            // search each local feature in KF1 associated to this node
            for(size_t i1 = 0, iend1 = f1it->second.size(); i1 < iend1; i1++) {
                const size_t idx1 = f1it->second[i1];
...
int ORBmatcher::SearchBySim3(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint*> &vpMatches12,
    ...
    // Check agreement, bidirectional check
    int nFound = 0;
